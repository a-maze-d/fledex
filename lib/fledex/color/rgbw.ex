...
defmodule Fledex.Color.RGBW do
  ...
  this module allows to also set the white (`W`) led commonly found in modern led strips
  (e.g. [WS2813B-RGBW](https://www.ledyilighting.com/wp-content/uploads/2025/02/WS2813-RGBW-datasheet.pdf)).

  It also provides the necessary conversion functions to the 0xwwrrggbb ([`colorint`](`t:Fledex.Color.Types.colorint/0`)) and`{r,g,b}` (tuple) representations.
  """
  import Bitwise
  ...
  @type t :: %__MODULE__{r: byte(), g: byte(), b: byte(), w: byte()}
  ...
  defimpl Fledex.Color do
    ...
    def to_colorint(%RGBW{r: r, g: g, b: b, w: w} = _rgbw) do
      (min(w, @max_value) <<< 24) + (min(r, @max_value) <<< 16) + (min(g, @max_value) <<< 8) +
        min(b, @max_value)
    end
  end
