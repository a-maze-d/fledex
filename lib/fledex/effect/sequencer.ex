...
defmodule Fledex.Effect.Sequencer do
  ...
  def apply(leds, count, config, triggers) do
    ...
    {triggers, effect_state} = set_next_sequence(sequences, triggers, trigger_name, current_sequence, effect_state)
  ...
  @spec get_module([{module, keyword}], {pos_integer, pos_integer | nil}) :: {module, keyword} | nil
  defp get_module(modules, {_sequence_id, module_index} = _current_sequence) do
    Enum.at(modules, module_index)
  end

  defp set_next_sequence(sequences, triggers, trigger_name, {sequence_id, _module_index} = _current_sequence, effect_state) do
